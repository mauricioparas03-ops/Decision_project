...
def multiSP_policy(state):
    ...
    # ── Min uptime diagnostic ─────────────────────────────────────────────────
# Before solving: show how many stage-1 nodes would have Vent forced
# if root starts ventilation (Vstart[0]=1 → 2 descendant stages must be ON)
    stage1_nids = children[0]
    stage2_nids = [ch for nid in stage1_nids for ch in children[nid]]
    print(f"  [UPTIME DIAG] U_vent=3 | stage-1 nodes={len(stage1_nids)} | "
        f"stage-2 nodes={len(stage2_nids)} | "
        f"If Vstart[0]=1: forces Vent=1 on ALL {len(stage1_nids)+len(stage2_nids)} descendant nodes")

    # ── Ventilation forcing diagnostic ───────────────────────────────────────
    vc = state['vent_counter']
    H  = state['H']
    v_eff_forced = (H > DATA['humidity_threshold']) or (vc in [1, 2])
    print(f"  [VENT DIAG] vent_counter={vc} | H={H:.1f} | Hhigh={DATA['humidity_threshold']:.1f} | "
        f"v_eff_forced={v_eff_forced} | "
      f"{'⚠ environment will force Vent=1 but MILP root is FREE' if v_eff_forced and vc in [1,2] and H <= DATA['humidity_threshold'] else 'OK'}")
    ...
    # After solving — check what the solver decided on ventilation
    from pyomo.environ import value
    vent_root = int(round(float(value(model.Vent[0]))))
    vstart_root = int(round(float(value(model.Vstart[0]))))
    s1_vents = [int(round(float(value(model.Vent[nid])))) for nid in children[0]]
    print(f"  [UPTIME DIAG] Vent[0]={vent_root} Vstart[0]={vstart_root} | "
        f"stage-1 Vent values={s1_vents} | "
        f"vent_counter_in={state['vent_counter']}")

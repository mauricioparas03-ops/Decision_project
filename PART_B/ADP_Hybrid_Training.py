...
data = get_fixed_data()
feature_cols = [
                "T1", "T2", "H", "price_t", "price_previous",
                "Occ1", "Occ2", "vent_counter", "low_override_r1", "low_override_r2"
                ]
...
def solve_bellman_equation_milp(state, next_t_weights):
    ...
    m.T1_next = Var(); m.T2_next = Var(); m.H_next = Var()
    m.ov1_next = Var(domain=Binary); m.ov2_next = Var(domain=Binary)
    ...
    # Overrule next state Room 1
    u_prev_r1 = state['low_override_r1']
    m.c_ylow_r1_a = Constraint(expr=m.T1_next <= data['temp_min_comfort_threshold'] + eps + M*(1 - m.y_low_r1))
    ...
    m.c_ov1_b = Constraint(expr=m.ov1_next <= u_prev_r1 + m.y_low_r1)
    m.c_ov1_c = Constraint(expr=m.ov1_next >= u_prev_r1 - m.y_ok_r1)
    m.c_ov1_d = Constraint(expr=m.ov1_next <= 1 - m.y_ok_r1)

    # Overrule next state Room 2
    u_prev_r2 = state['low_override_r2']
    m.c_ylow_r2_a = Constraint(expr=m.T2_next <= data['temp_min_comfort_threshold'] + eps + M*(1 - m.y_low_r2))
    ...
    m.c_ov2_b = Constraint(expr=m.ov2_next <= u_prev_r2 + m.y_low_r2)
...
for i in range(ITERATIONS_I):
    ...
    epsilon_i = max(0.05, 0.3 * (1 - i / ITERATIONS_I))
    ...
    for t in range(T_HOURS):
        ...
        for n in range(N_SAMPLES):
            ...
            is_random = np.random.rand() < epsilon_i
            if is_random:
            ...
            visited_states_actions[t].append((state_n.copy(), action, is_random))
    ...
    for j in range(SWEEPS_J):
        ...
        for t in reversed(range(T_HOURS)):
            ...
            for state_n, action_n, is_random in visited_states_actions[t]:
                if is_random:
                    continue
